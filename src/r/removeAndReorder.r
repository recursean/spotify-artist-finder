...
" 2018-04-17 00:00:00' and '", #format(Sys.Date(), "%Y-%m-%d"), 
                            " 2018-04-17 23:59:59') as a join artist_info as b on a.id = b.id left join (select distinct * from genres)  as c on b.id = c.id order by a.score desc);"))
